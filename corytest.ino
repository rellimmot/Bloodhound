...
void setup() {
  ...
  delay(100);
  digitalWrite(TRANSMIT,LOW);
  while(digitalRead(RECEIVE)!=HIGH){
    while(digitalRead(UP))   {M1Motor->run(FORWARD) ;  M3Motor->run(BACKWARD);} //forward
    while(digitalRead(DOWN)) {M1Motor->run(BACKWARD);  M3Motor->run(FORWARD) ;} //backward
    while(digitalRead(RIGHT)){M2Motor->run(FORWARD) ;  M4Motor->run(BACKWARD);} //right
    while(digitalRead(LEFT)) {M2Motor->run(BACKWARD);  M4Motor->run(FORWARD) ;} //left
    M1Motor->run(RELEASE); M2Motor->run(RELEASE); M3Motor->run(RELEASE); M4Motor->run(RELEASE);
  }
...
void offset_camera(){
  M1Motor->setSpeed(M1Speed); M3Motor->setSpeed(M3Speed);
  int goal = 850 + 215;//goal distance based on multiple parameter from blocks between wall
  unsigned int AlphaR_time = AlphaR.ping_median(5); // Send ping, get ping time in microseconds (uS).
  unsigned int AlphaL_time = AlphaL.ping_median(5); // Send ping, get ping time in microseconds (uS).
  //difference from goal and absolute value for error correction  
  int differenceL = AlphaL_time - goal; int differenceR = AlphaR_time - goal;  
